...
4. **用户主动声明该方向在某个非终末阶段就已结束** ⭐⭐
   触发语：「止步于此」「到此为止」「就这样结束了」「没进下一阶段」「淘汰了」「就到这里」
   例如原 phases=[报名/学习/初赛/复赛/决赛]，用户说"复赛进 32 强结束了没进决赛" → 必须 evolve：
   - 砍掉用户没到的阶段（决赛）
   - 把用户实际止步的阶段（复赛）变成最后一阶段
   - 这样阶段路径才反映真实结束状态，下一步成就才能正确触发"问鼎/收官"
   - proposed_phases 应该比 current_phases 少 1+ 个，并且最后一阶段名字要带"终结/收官"语义（如"复赛收官"、"止步复赛"），或保留原名只是顺序变成最后

evolve 时给出 proposed_phases (3-6个) 和 entry_remapping（说明旧阶段索引如何映射到新阶段索引）。
phase_index_after_evolve 必须等于新 proposed_phases 的最后一项（让用户落到"已收官"的位置）。
...
- 应当归类（不可 skip）的例子：
  - 行为："今天早起了" / "做了一组俯卧撑" / "读了 3 章书"
  - 方向声明："体测要到了" / "决定参加 XX 竞赛" / "下学期要研究 ZZ" / "打算考 PMP" → create 新维度起步阶段
  - **模糊推测也算**："估计保研夏令营快开始报名了" / "听说 XX 比赛要开了" / "应该差不多要交开题了" → create / update 起步阶段
  - 计划+具体目标："打算下个月之前完成 XX"
- 用户主动追踪一件事就有意义；维度没有"重要"门槛
- 真正应当 skip 的例子（纯感受、无行为无方向）："今天天气真好"、"心情不太好"、"食堂菜难吃"
